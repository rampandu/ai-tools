...
const POST_META = {
  ...
  "sql-interview-questions-complete-guide": { title: "SQL Interview Questions: The Complete Guide", description: "Every SQL question TCS, Infosys, Wipro, Accenture, and Capgemini actually ask, organized by topic — JOINs, keys, normalization, Nth highest salary." },
  ...
  "sql-optimization-techniques-for-large-tables": { title: "7 SQL Optimization Techniques for Large Tables", description: "Speed up slow queries on multi-million row tables: sargable WHERE clauses, JOIN order, the N+1 query problem, partitioning trade-offs, and reading EXPLAIN output." },
  ...
  "regex-cheat-sheet-for-backend-developers": { title: "Regex Cheat Sheet for Backend Developers (Node & Python)", description: "A regex cheat sheet for backend developers: symbols, character classes, a validation pattern reference table, and working Node.js and Python examples." },
  ...
  "cron-expression-complete-guide": { title: "Cron Expression Guide: Syntax, Fields & Platforms", description: "Every cron field explained, 10 ready-to-use examples, real application patterns, a platform comparison table, common mistakes, and a troubleshooting checklist." },
  ...
  "encode-images-base64-data-uri-html-css": { title: "Base64 Image Encoding in HTML/CSS: When It Helps", description: "Embed images directly in HTML and CSS with Base64 data URIs and skip the HTTP request — when it helps, when it hurts performance, and code examples." },
  ...
  "free-mysql-query-generator-online": { title: "Free MySQL Query Generator & Builder Online — No Signup", description: "Free MySQL query generator and builder online. Describe what you need in plain English and get a working MySQL query instantly — no signup, unlimited use." },
  ...
  "sql-query-for-pagination-limit-offset": { title: "SQL Pagination Guide — LIMIT/OFFSET vs Keyset (Copy-Paste SQL)", description: "SQL pagination explained: LIMIT/OFFSET syntax, why deep OFFSET pages get slow, and the keyset (cursor) pattern that keeps every page fast — with examples." },
  ...
  "cron-expression-examples-every-5-minutes": { title: "Cron Every 5 Minutes: Expression + Examples", description: "Every 5, 10, 15 and 30-minute cron patterns, field by field — plus the overlap and scheduler-delay gotchas that break them in production." },
  ...
  "debugging-cron-jobs-that-are-not-running": { title: "Cron Job Not Running? 5 Fixes That Actually Work", description: "Cron job silently not firing? The 5 most common causes — dead service, bad syntax, missing permissions, and PATH issues — with fixes for each." },
  ...
  "encode-decode-query-strings-nodejs": { title: "Node.js Query Strings: Encode & Decode the Right Way", description: "Stop hand-rolling URL encoding. Build and parse Node.js query strings correctly with URLSearchParams — arrays, special characters, and pitfalls." },
  ...
  "json-to-csv-conversion-guide": { title: "JSON to CSV in JavaScript: Working Code + 4 Libraries", description: "Convert JSON arrays to CSV with a copy-paste JavaScript function that handles RFC 4180 escaping, plus 4 popular libraries and 5 pitfalls to avoid." },
  ...
  "json-serialization-python-guide": { title: "Python JSON Serialization: Fix the Not-Serializable Error", description: "Stuck on Python's not-JSON-serializable TypeError? Two working fixes — a default function and a custom JSONEncoder — plus datetime and Decimal examples." },
  ...
  "debugging-memory-leaks-in-nodejs": { title: "How to Find a Node.js Memory Leak: Step-by-Step", description: "Node.js memory keeps climbing and never drops? Catch the leak with heap snapshots and --inspect, plus the two culprits behind most Node.js leaks." },
  ...
  "common-git-errors-and-how-to-fix-them": { title: "7 Common Git Errors and How to Fix Them Fast", description: "Stuck on a Git error? Step-by-step fixes for merge conflicts, detached HEAD, unrelated histories, and failed pushes, with commands you can copy." },
  ...
  "sql-join-types-explained-with-diagrams": { title: "SQL JOIN Types Explained: INNER, LEFT, RIGHT, FULL (Diagrams)", description: "See exactly which rows each SQL JOIN type returns — INNER, LEFT, RIGHT, FULL, CROSS, SELF — with diagrams, one shared example, and the exact result set for each." },
  "cron-expression-explainer-guide-how-it-works": { title: "Cron Expression Explainer Guide: How to Read the Output", description: "A worked example showing how the Cron Expression Explainer turns a cron string into a plain-English sentence and next-run list, plus why timezone changes the answer." },
  "sql-query-visualizer-guide-how-it-works": { title: "SQL Query Visualizer Guide: How to Read the Diagram", description: "A worked example showing how the SQL Query Visualizer turns a 3-table query into a join diagram and execution order, plus what an unlabeled join edge means." },
  "json-diff-viewer-guide-how-it-works": { title: "JSON Diff Viewer Guide: How Structural Comparison Works", description: "Why a JSON diff isn't the same as a text diff, a worked example showing added/removed/changed paths, and what the array-by-index limitation means for your results." },
  ...
  "how-to-decode-a-jwt-token-safely": { title: "How to Decode a JWT Token Safely", description: "Decode a JWT token safely in JavaScript, Python, or your browser — the three base64url parts, why decoding isn't verifying, and protecting live tokens." },
  ...
  "jwt-expiry-claims-exp-iat-nbf-explained": { title: "JWT exp, iat & nbf Explained — Avoid the Silent Expiry Bugs", description: "JWT exp, iat, and nbf explained: why they're unix seconds not milliseconds, how clock skew silently rejects valid tokens, and picking a safe token lifetime." },
  ...
  "md5-vs-sha256-which-hash-should-you-use": { title: "MD5 vs SHA-256 — Which Hash Should You Use?", description: "MD5 vs SHA-256 compared: speed vs security, the collision attacks that broke MD5 and SHA-1, when MD5 is still OK for checksums, and why passwords need Argon2." },
  ...
  "hash-collisions-explained": { title: "Hash Collisions Explained — Birthday Paradox & Broken Hashes", description: "Hash collisions explained: the pigeonhole principle, the birthday paradox, how collisions broke MD5 and SHA-1 in the real world, and why SHA-256 still holds." },
  ...
  "how-to-compare-two-text-files-online": { title: "How to Compare Two Text Files Online (Safely)", description: "Compare two text files online: use cases, how line diffs work, why client-side tools are safer for privacy, and CLI alternatives like diff and git diff." },
  ...
  "how-diff-algorithms-work-lcs-explained": { title: "How Diff Algorithms Work: LCS Explained", description: "The algorithm behind diff tools: longest common subsequence with a worked matrix, the Myers algorithm, and why moved blocks show as delete+add." },
  ...
  "milliseconds-vs-seconds-timestamp-bugs": { title: "Milliseconds vs Seconds: Fix the 1000x Bug", description: "Dates showing 1970 or year 56789? Spot the milliseconds vs seconds bug, fix JWT exp pitfalls, dodge the year-2038 problem, and copy a normalization function." },
  ...
  "common-yaml-errors-in-kubernetes-and-ci": { title: "4 Common YAML Errors in Kubernetes & CI (and Fixes)", description: "Fix the 4 YAML mistakes that break Kubernetes and CI — bad indentation, tab characters, unquoted 3.10 becoming a float, and missing --- separators." },
  ...
  "github-flavored-markdown-guide": { title: "GFM (GitHub Flavored Markdown) Guide — Tables & Alerts", description: "Full GFM (GitHub Flavored Markdown) reference: tables, task lists, alerts, footnotes, and code fences — with copy-paste examples for READMEs, issues, and PRs." },
  "markdown-for-technical-documentation-tips": { title: "Markdown for Technical Docs — 7 Tips for Readable Docs", description: "Practical Markdown tips for technical docs: heading hierarchy, code block language tags, relative links, alt text, tables vs lists, and docs-as-code CI." },
  "markdown-vs-html-when-to-use-which": { title: "Markdown vs HTML: The 80/20 Rule for When to Use Each", description: "Markdown covers 80% of writing needs with 20% of the effort; HTML gives full control. When each wins, how inline HTML in Markdown works, and a rule of thumb." },
  ...
  "generate-uuids-javascript-python-sql": { title: "How to Generate UUIDs in JavaScript, Python, and SQL", description: "Copy-paste UUID recipes: crypto.randomUUID() in JS, Python uuid4/uuid5, MySQL UUID_TO_BIN, PostgreSQL gen_random_uuid(), and efficient BINARY(16) storage." },
